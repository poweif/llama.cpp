            ...
            for (int64_t s = 0; s < n_seqs_unq; ++s) {
                ...
                for (int64_t i1 = 0; i1 < n_tps; ++i1) {
                    ...
                    for (int64_t i0 = 0; i0 < n_tps; ++i0) {
                        ...
                        blk[i1 * n_tps + i0] = llama_cast<T>(hparams.use_alibi ? -std::abs(p0 - p1) : 0.0f);
                    }
                }
            }
        } else {
...
llm_graph_context::llm_graph_context(const llm_graph_params & params) :
    ...
    mctx             (params.mctx),
    cross            (params.cross),
    mctx_tgt         (params.mctx_tgt),
    target_tok_embd  (params.target_tok_embd),
...
llm_graph_input_attn_no_cache * llm_graph_context::build_attn_inp_no_cache() const {
    ...
    // flash attention requires an f16 mask; when equal_seqs each sequence is its own independent
    // stream: use [n_tps, n_tps, 1, n_seqs_unq], otherwise [n_tokens, n_tokens, 1, 1].
    ...
    inp->self_kq_mask = ggml_new_tensor_4d(ctx0, type_mask, n_tps, n_tps, 1, n_streams);
    ggml_set_input(inp->self_kq_mask);

    inp->self_kq_mask_cnv = inp->self_kq_mask;

    if (hparams.swa_type != LLAMA_SWA_TYPE_NONE) {
        inp->self_kq_mask_swa = ggml_new_tensor_4d(ctx0, type_mask, n_tps, n_tps, 1, n_streams);
