...
void llama_kv_cache::set_input_pos_bucket(ggml_tensor * dst, const llama_ubatch * ubatch) const {
    ...
    for (int i = 0; i < n_tokens; ++i) {
        // For multi-stream (equal_seqs), each token belongs to exactly one sequence.
        // Map it to the owning stream so the correct KV cells are used.
        // For single-stream, seq_to_stream always maps to 0.
        GGML_ASSERT(ubatch->n_seq_id[i] >= 1);
        const llama_seq_id seq_id = ubatch->seq_id[i][0];
        const auto & cells = v_cells[seq_to_stream[seq_id]];

        for (int j = 0; j < n_kv; ++j) {
            // the position when the cell is empty is irrelevant — masked out during attention
