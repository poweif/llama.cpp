    ...
    if (format.section_start.empty() && !format.per_call_start.empty()) {
        ...
        if (is_qwen_per_call_style) {
            ...
            auto p1 = p.standard_json_tools(
                ...
                format.id_field, format.gen_id_field, format.parameter_order, format.openai_wrapper_trigger);
...
common_peg_parser analyze_tools::build_func_parser(common_chat_peg_builder & p, const std::string & name,
    ...
    if (!function.close.empty()) {
        ...
        auto close_peek = arguments.tolerate_intertag_whitespace
                              ? p.peek(p.space() + p.literal(effective_end))
                              : p.peek(p.literal(effective_end));
